...
#解决
#自定义某个属性的一种简单方法是将他定义为一个property。例如，下面的代码定义了一个property，增加对一个属性简单的类型检查。
import math
class Person:
	def __init__(self,first_name):
		sefl._first_name=first_name
	#getter function
	@property
	def first_name(self):
...
#第一个方法是一个getter函数，他使得first_name成为一个属性。
#其他两个方法给fitst_name属性添加了setter和deleter函数。
#需要强调的是只有在first_name属性被创建后，后面的两个装饰器@first_name.setter和@first_name.deleter才能被定义。
#
#property的一个关键特征是他看上去跟普通的attribute没什么两样，但是访问他的时候会自动触发getter、setter和deleter方法。例如：
a=Person('Guido')
...
#因此，在get和set方法中，你会看到对_first_name属性的操作，这也是实际数据保存的地方。
#另外，你可能还会问为什么__init__()方法中设置了self.first_name而不是self._first_name。
...
#通常来讲，你不会直接调取fget或者fset，它们会在访问property的是偶自动被触发
#
#只有当你确实需要对attribute执行其他额外的操作的时候才应该使用到property。
#有时候一些从其他编程语言(比如java)过来的程序员总任务所有的访问都应该通过getter和setter，所以它们认为代码应该像下面这样写：
class Person:
	def __init__(self, first_name):
